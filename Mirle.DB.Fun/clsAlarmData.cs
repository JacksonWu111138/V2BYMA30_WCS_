...
namespace Mirle.DB.Fun
{
    public class clsAlarmData
    {
            ...
            string happenTime, DataBase.DB db, ref string strEM)
        {
            ...
            {
                string strSql = $"select * from {Parameter.clsAlarmData.TableName} where " +
                    $"{Parameter.clsAlarmData.Column.AlarmCode} = '{alarmCode}' ";
                strSql += $" and {Parameter.clsAlarmData.Column.AlarmSts} = '{Parameter.clsAlarmData.Status.Occur}' and " +
                    $"{Parameter.clsAlarmData.Column.DeviceID} = '{deviceId}' and {Parameter.clsAlarmData.Column.JobID} = '{jobId}' ";

                int iRet = db.GetDataTable(strSql, ref dtTmp, ref strEM);
                if (iRet == DBResult.Success) return true;
                else if (iRet == DBResult.NoDataSelect)
                {
                    strSql = $"insert into {Parameter.clsAlarmData.TableName}({Parameter.clsAlarmData.Column.JobID}," +
                        $"{Parameter.clsAlarmData.Column.DeviceID},{Parameter.clsAlarmData.Column.BufferID}," +
                        $"{Parameter.clsAlarmData.Column.AlarmCode},{Parameter.clsAlarmData.Column.AlarmDef}," +
                        $"{Parameter.clsAlarmData.Column.AlarmSts},{Parameter.clsAlarmData.Column.Start_Date}," +
                        $") values(";
                    strSql += $"'{jobId}', '{deviceId}', '{bufferId}', '{alarmCode}', '{alarmDef}',"+
                        $"'{Parameter.clsAlarmData.Status.Occur}', '{happenTime}')";
                    if (db.ExecuteSQL(strSql, ref strEM) == DBResult.Success) return true;
                    else
                    {
                        clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Error, $"{strSql} => {strEM}");
            ...
            string happenTime, DataBase.DB db, ref string strEM)
        {
            ...
            {
                string strSql = $"select * from {Parameter.clsAlarmData.TableName} where " +
                    $"{Parameter.clsAlarmData.Column.AlarmCode} = '{alarmCode}' and {Parameter.clsAlarmData.Column.AlarmSts} = "+
                    $"'{Parameter.clsAlarmData.Status.Occur}' and {Parameter.clsAlarmData.Column.DeviceID} = '{deviceId}' " + 
                    $"and {Parameter.clsAlarmData.Column.JobID} = '{jobId}'";
                ...
                if (iRet == DBResult.Success)
                {
                    ...
                    $"{Parameter.clsAlarmData.Column.AlarmCode} = '{alarmCode}' and {Parameter.clsAlarmData.Column.AlarmSts} = " +
                    $"'{Parameter.clsAlarmData.Status.Occur}' and {Parameter.clsAlarmData.Column.DeviceID} = '{deviceId}' " + 
                    $"{Parameter.clsAlarmData.Column.JobID} = '{jobId}' ";
                    if (db.ExecuteSQL(strSql, ref strEM) == DBResult.Success) return true;
                    else
                    {
                        clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Error, $"{strSql} => {strEM}");
