...
namespace Mirle.DB.Fun
{
    public class clsMiddleCmd
    {
        ...
        public int GetMiddleCmd_ByBatchID(string sBatchID, ref DataTable dtTmp, DataBase.DB db)
        {
            try
            {
                string strSql = $"select * from {Parameter.clsMiddleCmd.TableName} where " +
                    $"{Parameter.clsMiddleCmd.Column.BatchID} = '{sBatchID}'";
                dtTmp = new DataTable(); string strEM = "";
                int iRet = db.GetDataTable(strSql, ref dtTmp, ref strEM);
                if (iRet != DBResult.Success) clsWriLog.Log.FunWriLog(WriLog.clsLog.Type.Error, $"{strSql} => {strEM}");
