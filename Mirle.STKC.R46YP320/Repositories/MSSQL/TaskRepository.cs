...
namespace Mirle.STKC.R46YP320.Repositories.MSSQL
{
    public class TaskRepository : ITaskRepository
    {
        ...
        public TaskRepository(MSSqlDbService dbService)
        {
        ...
        {
            var sql = @"insert into Task(DeviceID,CommandID,TaskNo,CraneNo,ForkNo,CSTID,TaskState,CMDState,
...
values(@StockerID, @CommandID, @TaskNo, @CraneNo, @ForkNo, @CSTID, @TaskState, @CMDState,
@TransferMode, @Source, @SourceBay, @Destination, @DestinationBay,
@TravelAxisSpeed, @LifterAxisSpeed, @RotateAxisSpeed, @ForkAxisSpeed,
@UserID, @BCRReadFlag, @Priority, @QueueDT,@NextDest)";
        ...
        {
            var sql = @"update Task set TaskState=@TaskState
        ...
        {
            var sql = "select * from Task where DeviceID =@StockerID and TaskState =@TaskState";
            using (var con = _dbService.GetDbConnection())
            {
        ...
        {
            var sql = @"update Task set TaskState=@TaskState,CMDState=@CMDState,InitialDT='',RenewFlag='Y' where TaskNo=@TaskNo";
            using (var con = _dbService.GetDbConnection())
            {
        ...
        {
            var sql = @"insert into HisTask(HisDT,DeviceID,CommandID,TaskNo,CraneNo,CSTID,TaskState,CompleteCode,CMDState,
...
UserID,LotId,CSTType,BCRReadFlag,BCRReadDT,BCRReplyCSTID,BCRReadStatus,Priority,QueueDT,
InitialDT,WaitingDT,ActiveDT,FinishDT,FinishLocation,C1StartDT,CSTOnDT,CSTTakeOffDT,C2StartDT,T1,T2,T3,T4,
F1StartDT,F2StartDT,RenewFlag,TravelDistance,NextDest)
values(@HisDT, @StockerID, @CommandID, @TaskNo, @CraneNo, @CSTID, @TaskState, @CompleteCode, @CMDState,
@TransferMode, @TransferModeType, @Source, @SourceBay, @Destination, @DestinationBay, @AtSourceDT, @AtDestinationDT,
@TravelAxisSpeed, @LifterAxisSpeed, @RotateAxisSpeed, @ForkAxisSpeed, @CmdInfo,
@UserID, @LotId, @CSTType, @BCRReadFlag, @BCRReadDT, @BCRReplyCSTID, @BCRReadStatus, @Priority, @QueueDT,
@InitialDT, @WaitingDT, @ActiveDT, @FinishDT, @FinishLocation, @C1StartDT, @CSTOnDT, @CSTTakeOffDT, @C2StartDT, @T1, @T2, @T3, @T4,
