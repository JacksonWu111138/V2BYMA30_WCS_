...
{
    public partial class CtrlEmptyBinLoadDone : Form
    {
        ...
        private void button_EmptyBinLoadDone_Click(object sender, EventArgs e)
        {
            bool ctrltype = true;
            ...
            if (ctrltype)
            {
                EmptyBinLoadDoneInfo info = new EmptyBinLoadDoneInfo
                {
                    jobId = textBox_jobId.Text,
                    location = textBox_location.Text
                };
                if (!clsAPI.GetAPI().GetEmptyBinLoadDone().FunReport(info, Apiconfig.IP))
                {
                    MessageBox.Show($"失敗, jobId:{info.jobId}.", "Empty Bin Load Done", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    MessageBox.Show($"成功, jobId:{info.jobId}.", "Empty Bin Load Done", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
    }
